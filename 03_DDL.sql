...
CREATE VIEW EMP10_SIMPLE_VIEW AS
...
-- 직원 테이블에서 10번 부서원중에서 7782 직원을 삭제
...
-- VIEW에서 7932 직원의 급여를 10000으로 변경
UPDATE EMP10_SIMPLE_VIEW SET SAL = 10000 WHERE EMPNO = 7934;

-- 뷰 삭제 : DDL (TCL AUTO COMMIT)
DROP VIEW EMP10_SIMPLE_VIEW;

-- 현재 상황에서 ROLLBACK을 하면 삭제한 10번 부서원은 복구될까? - 안됨
ROLLBACK;